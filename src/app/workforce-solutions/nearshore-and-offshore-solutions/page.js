...
export default function GlobalSolutionsPage() {
  ...
  return (
    <div className="bg-[#f9faed] min-h-screen md:pt-12 selection:bg-[#1b4c53] selection:text-white font-sans">
      ...
      <section className="relative h-[90vh] flex items-center justify-center overflow-hidden px-6">
        ...
        <div className="relative z-10 text-center max-w-4xl">
          ...
          <h1 className="text-8xl md:text-[10rem] font-black text-[#1b4c53] leading-[0.85] tracking-tighter">
            GLOBAL <br />{" "}
            <span className="italic font-light text-[#3a7e88]">REACH.</span>
          </h1>

          <p className="mt-10 font-[Urbanist] mb-18 text-xl text-[#1b4c53]/70 font-medium max-w-xl mx-auto leading-relaxed">
            ...
            >
              ...
              <div className="space-y-8">
                {[
                  {
                    label: "Nearshore",
                    text: "Same-day collaboration, shared culture, and high-velocity output.",
                  },
                  {
                    label: "Offshore",
                    text: "Round-the-clock development, high-volume scalability, and cost optimization.",
                  },
                ].map((item, i) => (
                  <div key={i} className="flex gap-4">
                    <CheckCircle2
                      className="text-[#3a7e88] shrink-0"
                      size={24}
                    />
                    <div>
                      <h4 className="text-[#f9faed] font-bold text-lg">
                        {item.label}
                      </h4>
                      <p className="text-[#f9faed]/50 text-sm leading-relaxed font-[Urbanist]">
                        {item.text}
                      </p>
                    </div>
                  </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {pillars.map((item, i) => (
                <motion.div
                  ...
                  whileHover={{
                    y: -10,
                    backgroundColor: "rgba(249, 250, 237, 0.04)",
                  }}
                ...
                >
                  <motion.div
                    animate={item.anim}
                    transition={{
                      duration: item.title === "Time-Zone Sync" ? 10 : 3,
                      repeat: Infinity,
                      ease: "linear",
                    }}
                  ...
                  <p className="text-[#f9faed]/40 text-sm leading-relaxed font-[Urbanist]">
                    {item.desc}
                  </p>
                </motion.div>
              ))}
            ...
            {[
              {
                ...
                detail:
                  "We don't believe in 'throw it over the wall' outsourcing. Our teams are integrated directly into your Slack, Jira, and culture.",
              },
              {
                title: "Global Compliance Hub",
                detail:
                  "Localized HR, payroll, and office space managed by us. You get the talent; we handle the international complexity.",
              },
            ].map((card, i) => (
              <motion.div
                key={i}
              ...
              >
                ...
                <div className="relative bg-[#f9faed] p-16 rounded-[3.9rem] h-full flex flex-col justify-between">
                  <div>
                    ...
                    <h3 className="text-3xl font-bold text-[#1b4c53] mb-6">
                      {card.title}
                    </h3>
      ...
      <section className="pb-32 px-2">
        <motion.div
          whileHover={{ y: -5 }}
          className="max-w-6xl mx-auto md:p-24 py-20 bg-[#1b4c53] rounded-[5rem] p-12 text-center text-[#f9faed] relative overflow-hidden shadow-2xl shadow-[#1b4c53]/40"
        >
          {/* Animated "Signal" Background */}
          <motion.div
            animate={{ scale: [1, 1.5], opacity: [0.3, 0] }}
            transition={{ duration: 4, repeat: Infinity }}
            className="absolute inset-0 m-auto pointer-events-none w-96 h-96 border border-[#3a7e88]/50 rounded-full"
          />
          ...
          <Link href="/contact-us" aria-label="Build My Global Team">
            <motion.div
              whileHover={{ scale: 1.05 }}
              className="inline-flex items-center gap-4 cursor-pointer bg-[#f9faed] text-[#1b4c53] md:px-14 md:py-7 px-10 py-4 rounded-full font-black text-md md:text-xl hover:shadow-[0_0_40px_rgba(249,250,237,0.2)] transition-all group "
            >
              Build My Global Team{" "}
